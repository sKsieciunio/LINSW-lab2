...
static void *button_thread(void *arg) {
  ...
  while (true) {
    ...
    bool value;
    if (gpio_read(gpio_btns[idx], &value) < 0)
      continue;
...
int main(void) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    if (gpio_open(gpio_leds[i], "/dev/gpiochip0", LED_PINS[i], GPIO_DIR_OUT) <
        0) {
      fprintf(stderr, "gpio_open LED%d (gpio%d): %s\n", i + 1, LED_PINS[i],
              gpio_errmsg(gpio_leds[i]));
      exit(1);
    }
  }
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    if (gpio_open(gpio_btns[i], "/dev/gpiochip0", BTN_PINS[i], GPIO_DIR_IN) <
        0) {
      fprintf(stderr, "gpio_open BTN%d (gpio%d): %s\n", i + 1, BTN_PINS[i],
              gpio_errmsg(gpio_btns[i]));
      exit(1);
    }
    if (gpio_set_edge(gpio_btns[i], GPIO_EDGE_BOTH) < 0) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    btn_idx[i] = i;
    if (pthread_create(&btn_threads[i], NULL, button_thread, &btn_idx[i]) !=
        0) {
